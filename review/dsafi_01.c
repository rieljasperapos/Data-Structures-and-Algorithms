...
ProdList populateProdList()
{
  ...
  // Write your code here
  ProdList retList = NULL;
  ProdList temp;
  ProdList *trav;
  int indx;
  for (indx = 0; indx < count; indx++) {
    for (trav = &retList; *trav != NULL; trav = &(*trav)->plink) {}
...
void displayProdList(ProdList L)
{
  ...
  while (L != NULL) {
    printf("\n%-7s", L->prod.prodID);
    printf("%-12s", L->prod.prodDesc.name);
    printf("%-15d", L->prod.prodDesc.weight);
    count++;
    L = L->plink;
  }
...
void makeListToSet(ProdList L)
{
  ...
  while (*trav != NULL) {
    visited = 0;
    for (indx = 0; indx < count; indx++) {
      if (strcmp((*trav)->prod.prodID, data[indx].prodID) == 0) {
        *trav = (*trav)->plink;
        visited = 1;
      }
    }
